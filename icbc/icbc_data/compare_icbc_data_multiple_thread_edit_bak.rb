...
module CompareData
    ...
    def self.start_compare old_db_path, new_db_path, book
        while File.exist?(old_db_path) && File.exist?(new_db_path) do
            ...
            puts changed_tables_data_strings
    ...
    def self.compare_create_for_table table_name, old_db_path, new_db_path, classified_table_name, is_update = false
        ...
        when /ICBC区|非ICBC区/
            if is_update
                # data_string = `sqlite3 #{old_db_path} "attach '#{new_db_path}' AS new_db; select b.view, b.name, b.ttl, b.type, b.rdata, a.view, a.name, a.ttl, a.type, a.rdata from new_db.'#{table_name}' a inner join '#{table_name}' b on a.id = b.id where (a.ttl <> b.ttl or a.rdata <> b.rdata) and (a.type <> 'SOA' or b.type <> 'SOA');"`
                data_string = `sqlite3 #{old_db_path} "attach '#{new_db_path}' AS new_db; select b.view, b.name, b.ttl, b.type, b.rdata, a.view, a.name, a.ttl, a.type, a.rdata from new_db.'#{table_n    ame}' a inner join '#{table_name}' b on a.id = b.id where a.ttl <> b.ttl or a.rdata <> b.rdata;"`
            else
                data_string = `sqlite3 #{old_db_path} "attach '#{new_db_path}' AS new_db; select a.view, a.name, a.ttl, a.type, a.rdata from new_db.'#{table_name}' a left join '#{table_name}' b on a.id = b.id where b.id is null and a.type <> 'SOA';"`
